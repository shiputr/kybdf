...
function setupResponsiveMenu() {
    ...
    if (!navMenu || !moreBtn || !moreDropdown) return;
    ...
    allItems.forEach((item, idx) => {
        ...
        if (link && link.classList.contains('login-btn')) {
            loginItem = item;
            loginIndex = idx;
        }
    });
    
    const normalItems = allItems.filter((_, idx) => idx !== loginIndex);
    ...
    const containerWidth = navFlex.clientWidth;
    
    moreBtn.style.display = 'flex';
    const moreBtnWidth = moreBtn.offsetWidth + 10;
    const loginWidth = loginItem ? loginItem.offsetWidth + 5 : 0;
    ...
    for (let i = 0; i < normalItems.length; i++) {
        ...
        if (totalWidth + itemWidth + loginWidth + moreBtnWidth < containerWidth - 40) {
    ...
    moreDropdown.querySelectorAll('.dropdown').forEach(dropdown => {
        dropdown.classList.add('more-dropdown-item');
...
window.addEventListener('load', () => {
    setupResponsiveMenu();
    setTimeout(setupResponsiveMenu, 100);
});
